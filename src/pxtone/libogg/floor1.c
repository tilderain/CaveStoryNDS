...
void floor1_free_info(vorbis_info_floor *i){
  ...
  if(info){
    if(info->class1)_ogg_free(info->class1);
...
static void mergesort(char *index,ogg_uint16_t *vals,ogg_uint16_t n){
  ogg_uint16_t i,j;
  char *temp,*A=index,*B=(char *)_ogg_malloc(n*sizeof(*B));
...
vorbis_info_floor *floor1_info_unpack (vorbis_info *vi,oggpack_buffer *opb){
  ...
  info->class1=
    (floor1class *)_ogg_malloc((maxclass+1)*sizeof(*info->class1));
  for(j=0;j<maxclass+1;j++){
    info->class1[j].class_dim=oggpack_read(opb,3)+1; /* 1 to 8 */
    info->class1[j].class_subs=oggpack_read(opb,2); /* 0,1,2,3 bits */
    if(oggpack_eop(opb)<0) goto err_out;
    if(info->class1[j].class_subs){
      info->class1[j].class_book=oggpack_read(opb,8);
      if(info->class1[j].class_book>=ci->books)goto err_out;
    }else
      info->class1[j].class_book=0;
    if(info->class1[j].class_book>=ci->books)goto err_out;
    for(k=0;k<(1<<info->class1[j].class_subs);k++){
      info->class1[j].class_subbook[k]=oggpack_read(opb,8)-1;
      if(info->class1[j].class_subbook[k]>=ci->books &&
	 info->class1[j].class_subbook[k]!=0xff)goto err_out;
    }
  }
  ...
  for(j=0,k=0;j<info->partitions;j++)
    count+=info->class1[info->partitionclass[j]].class_dim; 
  info->postlist=
...
ogg_int32_t *floor1_inverse1(vorbis_dsp_state *vd,vorbis_info_floor *in,
  ...
  if(oggpack_read(&vd->opb,1)==1){
    ...
    for(i=0,j=2;i<info->partitions;i++){
      ...
      int cdim=info->class1[classv].class_dim;
      int csubbits=info->class1[classv].class_subs;
      int csub=1<<csubbits;
      int cval=0;

      /* decode the partition's first stage cascade value */
      if(csubbits){
	cval=vorbis_book_decode(books+info->class1[classv].class_book,&vd->opb);

	if(cval==-1)goto eop;
      }

      for(k=0;k<cdim;k++){
	int book=info->class1[classv].class_subbook[cval&(csub-1)];
	cval>>=csubbits;
	if(book!=0xff){
