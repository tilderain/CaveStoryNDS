...
*/
    ...
    // added by Moonlight. (2007/01/13)
    // reffer http://utorrent.com/tremorleak.html
    if(!bs->outstanding){
      _ogg_free(bs);
    }
  }
...
static ogg_reference *_fetch_ref(ogg_buffer_state *bs){
  ...
  if(bs->unused_references){
    orf=bs->unused_references;
    bs->unused_references=orf->next;
  }else{
    /* allocate a new reference */
    orf=(ogg_reference *)_ogg_malloc(sizeof(*orf));
  }
...
static ogg_reference *ogg_buffer_alloc(ogg_buffer_state *bs,long bytes){
  ...
  ogg_reference *orf=_fetch_ref(bs);
  orf->buffer=ob;
  return orf;
}

/* enlarge the data buffer in the current link */
static void ogg_buffer_realloc(ogg_reference *orf,long bytes){
  ogg_buffer    *ob=orf->buffer;
...
static void _ogg_buffer_mark_one(ogg_reference *orf){
  orf->buffer->refcount++;
}

/* increase the refcount of the buffers to which the reference points */
static void ogg_buffer_mark(ogg_reference *orf){
  while(orf){
    _ogg_buffer_mark_one(orf);
    orf=orf->next;
  }
}
...
static ogg_reference *ogg_buffer_sub(ogg_reference *orf,long length){
  ogg_reference *ret=0,*head=0;
  
  /* duplicate the reference chain; increment refcounts */
  while(orf && length){
    ogg_reference *temp=_fetch_ref(orf->buffer->ptr.owner);
    if(head)
      head->next=temp;
    else
      ret=temp;
    head=temp;
    head->buffer=orf->buffer;    
    head->begin=orf->begin;
    head->length=length;
    if(head->length>orf->length)
      head->length=orf->length;
...
ogg_reference *ogg_buffer_dup(ogg_reference *orf){
  ogg_reference *ret=0,*head=0;
  /* duplicate the reference chain; increment refcounts */
  while(orf){
    ogg_reference *temp=_fetch_ref(orf->buffer->ptr.owner);
...
/* split a reference into two references; 'return' is a reference to
   ...
   split.  If pos is at orf past the end of the passed in segment,
   'head/tail' are NULL */
static ogg_reference *ogg_buffer_split(ogg_reference **tail,
  ...
  ogg_reference *orf=*tail;

  while(orf && pos>orf->length){
    pos-=orf->length;
    orf=orf->next;
  }
  ...
  if(!orf || pos==0){
    ...
    if(pos>=orf->length){
      ...
      long lengthA=pos;
      long beginB=orf->begin+pos;
      long lengthB=orf->length-pos;
      
      /* make a new reference to tail the second piece */
      *tail=_fetch_ref(orf->buffer->ptr.owner);
      
      (*tail)->buffer=orf->buffer;
      (*tail)->begin=beginB;
      (*tail)->length=lengthB;
      (*tail)->next=orf->next;
      _ogg_buffer_mark_one(*tail);
      if(head && orf==*head)*head=*tail;    
...
static void ogg_buffer_release_one(ogg_reference *orf){
  ogg_buffer *ob=orf->buffer;
  ...
  orf->next=bs->unused_references;
  bs->unused_references=orf;
...
static void ogg_buffer_release(ogg_reference *orf){
  while(orf){
    ogg_reference *next=orf->next;
    ogg_buffer_release_one(orf);
    orf=next;
  }
}

static ogg_reference *ogg_buffer_pretruncate(ogg_reference *orf,long pos){
...
static ogg_reference *ogg_buffer_walk(ogg_reference *orf){
  if(!orf)return NULL;
  while(orf->next){
    orf=orf->next;
  }
...
static int oggbyte_init(oggbyte_buffer *b,ogg_reference *orf){
  memset(b,0,sizeof(*b));
  if(orf){
    b->ref=b->baseref=orf;
    b->pos=0;
...
previous page, and spans to the next page (in the case of a three orf
...
unsigned char *ogg_sync_bufferin(ogg_sync_state *oy, long bytes){
  ...
  */
  ...
  {
    ogg_reference *newor=(ogg_reference *)ogg_buffer_alloc(oy->bufferpool,bytes);
    oy->fifo_head->next=newor;
    oy->fifo_head=newor;
  }
  return oy->fifo_head->buffer->data;
}
...
static ogg_uint32_t _checksum(ogg_reference *orf, int bytes){
  ogg_uint32_t crc_reg=0;
  int j,post;

  while(orf){
    unsigned char *data=orf->buffer->data+orf->begin;
    post=(bytes<orf->length?bytes:orf->length);
    for(j=0;j<post;++j)
...
long ogg_sync_pageseek(ogg_sync_state *oy,ogg_page *og){
     ...
     checksum */
  {
    ...
    if(chksum!=_checksum(oy->fifo_tail,oy->bodybytes+oy->headerbytes)){
      ...
      /* D'oh.  Mismatch! Corrupt page (orf miscapture and not a page
...
/* sync the stream and get a page.  Keep trying until we find a page.
   ...
   _stream, _clear, _init, orf _buffer */
...
static void _span_queued_page(ogg_stream_state *os){ 
  while( !(os->body_fill&FINFLAG) ){
    ...
    if(!os->header_tail){
      ...
      /* are we in sequence? */
      if(pageno!=os->pageno){
        if(os->pageno==-1) /* indicates seek orf reset */
