...
static void *_insert(void *ptr,long bytes,char *file,long line){
...
#ifdef _VDBG_GRAPHFILE
  {
    ...
    out=fopen(buffer,"a");
    _consolePrintf("%ld, %ld\n",-start_time+(tv.tv_sec*1000)+(tv.tv_usec/1000),
	    file_bytes[i]-(bytes-HEAD_ALIGN));
    _consolePrintf("%ld, %ld # FILE %s LINE %ld\n",
	    -start_time+(tv.tv_sec*1000)+(tv.tv_usec/1000),
...
static void _ripremove(void *ptr){
  ...
  if(pointers[insert]==NULL){
    _consolePrint("DEBUGGING MALLOC ERROR: freeing previously freed memory\n");
    _consolePrintf("\t%s %ld\n",((head *)ptr)->file,((head *)ptr)->line);
  }

  if(global_bytes<0){
    _consolePrint("DEBUGGING MALLOC ERROR: freeing unmalloced memory\n");
  }
...
void _VDBG_dump(void){
  ...
  for(i=0;i<ptop;i++){
    head *ptr=pointers[i];
    if(ptr)
      _consolePrintf("unfreed bytes from %s:%ld\n",
	      ptr->file,ptr->line);
  }
