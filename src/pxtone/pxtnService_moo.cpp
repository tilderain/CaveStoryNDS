...
bool pxtnService::_moo_PXTONE_SAMPLE( int smp_num )
{
	...
	if( _moo_fade_fade < 0 )
	{
		if( _moo_fade_count > 0  ) 
		{
			...
			if(_moo_fade_half)
			{
				if(_moo_fade_count < ((_moo_fade_max << 8) / 2))
				{
					_moo_fade_count = ((_moo_fade_max << 8) / 2);
				}
			}
			else if(_moo_fade_count < 0) _moo_fade_count = 0;
		}
		else
		{
			_moo_InitUnitTone();
...
bool pxtnService::moo_set_fade( int32_t  fade, float sec, bool half)
{
	if( !_moo_b_init ) return false;
...
bool pxtnService::moo_preparation( const pxtnVOMITPREPARATION *p_prep )
{
	...
	if( fadein_sec > 0 ) moo_set_fade( 1, fadein_sec , false);
	else                 moo_set_fade( 0,          0 , false);
